# ifdo_api/api/v1/fields/image_set_related_material.py
from fastapi import APIRouter
from ifdo_api.api.generic_router import generate_crud_router
from ifdo_api.crud.fields import image_set_related_material_crud
from ifdo_api.schemas.fields import ImageSetRelatedMaterialSchema

router: APIRouter = generate_crud_router(
    model_crud=image_set_related_material_crud,
    schema=ImageSetRelatedMaterialSchema,
    schema_create=ImageSetRelatedMaterialSchema,
)
